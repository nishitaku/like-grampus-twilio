...
exports.handler = function(context, event, callback) {
  ...
  body.events.forEach(async (webhookData) => {
    ...
    switch (msgEvtType) {
      ...
      case 'message': // メッセージ受信時
        const messageType = webhookData.message.type;
        switch (messageType) {
          case 'text': // テキストメッセージの場合
            botClient.replyMessage(replyToken, {type: 'text', text: 'textありがとう'});
            break;

          case 'image': // 画像の場合
            botClient.pushMessage(userId, {type: 'text', text: 'image読込中'});
            ...
            const filename = dateFns.format(new Date(), 'yyyyMMddHHmmss') + '_grampus.jpeg';
...
async function getLineImage(messageId, replyToken) {
  ...
  try{
    ...
    await botClient.replyMessage(replyToken, {type: 'text', text: '画像取得に成功しました'});
  } catch (err) {
    console.log(`getLineImage: error=${err}`);
    await botClient.replyMessage(replyToken, {type: 'text', text: '画像取得に失敗しました'});
  }
